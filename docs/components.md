...
| `SharingPage` | `src/components/SharingPage.tsx:43` | Owner **page** at `/sharing` (linked from the HomePage "Sharing" button, shown when you own ≥1 tree) rendered as a **people × trees access matrix**. An "Invite someone" bar above the table adds a draft person; each cell is a 3-state role chip-select (— / Viewer / Editor) that grants, updates, or revokes that person's access to that tree immediately via `useOwnerShares.setRole` (emerald=editor, cobalt=viewer, slate=none). Tree columns use initial-avatar badges; person rows use a round avatar + name/email + a green/amber Active/Pending dot; the first column is sticky. Same sticky header as Home (logo + brand + `AccountMenu`) with a back link. Loads `/api/shares`. |
